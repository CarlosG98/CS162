 ...
 * *********************************************************************/
void get_budget_data(budget* budget, int size, ifstream &f){
...
/*void swap_cats(transaction* xp, transaction* yp){
	struct transaction* temp = *xp;
	*xp = *yp;
	*yp = temp;
}
...
void sort_by_category(budget* b, int user_budget, int decide){
	int n = (b[user_budget].num_transactions)/(sizeof(b[user_budget].t[0].category));
	for(int i=0; i < n; i++){
		for(int j=0; j < n - i - 1; j++){
			if( b[user_budget].t[i].category[j] > b[user_budget].t[i+1].category[j]){
				swap_cats(&b[user_budget].t[i], &b[user_budget].t[i+1]);
